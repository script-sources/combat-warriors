...
Builder.new():library(library):windows({ Window.new():title("Muffet Hub | Combat Warriors"):centered(true):autoShow(true):withFadeTime(0):pages({ Page.new():title("Gameplay"):left({ Groupbox.new():title("Auto Parry"):elements({ Toggle.new("gameplay.auto_parry.enabled"):title("Enabled"):tooltip("Automatically parry attacks"):default(false):extensions({ KeyPicker.new("gameplay.auto_parry.key"):title("Auto Parry"):bind("V"):mode("Hold") }), DependencyBox.new():dependsOn("gameplay.auto_parry.enabled", true):elements({ Toggle.new("gameplay.auto_parry.predict_enabled"):title("Predict"):tooltip("Predicts the enemy's velocity, so you can parry in advance."):default(false), DependencyBox.new():dependsOn("gameplay.auto_parry.predict_enabled", true):elements({ Slider.new("gameplay.auto_parry.predict_time"):title("Amount"):suffix(" ms"):round(0):limits(10, 400):default(10):compact(true):hideMax(true) }), Toggle.new("gameplay.auto_parry.debug"):title("Debugger"):tooltip("Enable debug notifications for Auto Parry"):default(true) }) }) }):right({}), Page.new():title("Target"):left({ Groupbox.new():title("Selection"):elements({ MultiDropdown.new():index("target.selection.filters"):title("Filters"):tooltip("Only targets that meet these conditions will be considered"):options({ "Enemies", "Alive", "In Radius", "Visible", "Not Obstructed" }):default({ "Enemies", "Alive", "In Radius" }), Dropdown.new():index("target.selection.mode"):title("Priority"):tooltip("Prioritizes certain targets over others"):options({ "Closest to Cursor", "Closest to Player", "Lowest HP", "Highest HP" }):default("Closest to Player"), Slider.new("target.selection.fov_radius"):title("FOV Radius"):round(0):limits(10, 500):default(100):compact(true):hideMax(true):suffix("px") }) }):right({ Groupbox.new():title("Filters"):elements({ MultiDropdown.new("target.filter.players"):title("Player List"):tooltip("The list of players to whitelist/blacklist"):canNull(true):specialType("Player"), Dropdown.new("target.filter.players_type"):title("Disposition"):tooltip("Sets the selected players as allies or enemies"):options({ "Ally", "Enemy" }):default("Enemy"), Spacer.new(8), Toggle.new("target.filter.team_filter"):title("Filter teams?"):tooltip("Enables team checking for the filter"):default(false), DependencyBox.new():dependsOn("target.filter.team_filter", true):elements({ MultiDropdown.new("target.filter.teams"):title("Team List"):tooltip("The list of teams to whitelist/blacklist"):canNull(true):specialType("Team"), Dropdown.new("target.filter.teams_type"):title("Disposition"):tooltip("Sets the selected teams as allies or enemies"):options({ "Ally", "Enemy" }):default("Enemy"), Dropdown.new("target.filter.resolve"):title("Resolve Method"):tooltip("Sets how the filter will resolve conflicts in disposition."):options({ "Resolve as Ally", "Resolve as Enemy" }):default("Resolve as Ally") }) }) }), Page.new():title("Settings"):left({}):right({}) }) }):renderUI()
--[[
	***********************************************************
	 * INITIALIZATION
